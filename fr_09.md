...
Tính năng **Tải lên hình ảnh sân & Tích hợp SDK Bên thứ ba (FR-09)** cho phép Quản lý (`ROLE_MANAGER`) hoặc Khách hàng (`ROLE_CUSTOMER`) tải các tệp tin hình ảnh lên hệ thống đám mây Cloudinary thông qua API, hỗ trợ lưu trữ an toàn và trả về liên kết an toàn (Secure URL).
...
```mermaid
graph TD
    A[Client gửi yêu cầu POST /api/v1/manager/files/upload <br/> hoặc /api/v1/customer/files/upload] --> B[Kiểm tra tính hợp lệ của File]
...
*   **Method:** `POST`
*   **Path:** `/api/v1/manager/files/upload` hoặc `/api/v1/customer/files/upload`
*   **Access:** `ROLE_MANAGER`, `ROLE_CUSTOMER` (Đăng nhập tương ứng vào hệ thống)
*   **Headers:**
...
*   **Method:** `POST`
*   **Path:** `/api/v1/admin/files/upload/courts/{id}` hoặc `/api/v1/manager/files/upload/courts/{id}`
*   **Access:** `ROLE_ADMIN`, `ROLE_MANAGER` (Đăng nhập tương ứng vào hệ thống)
*   **Headers:**
    - `Content-Type`: `multipart/form-data`
    - `Authorization`: `Bearer <token>`
*   **Path Parameters:**
...
*   **Lỗi Không Tìm Thấy Sân Cầu hoặc Sân Đã Bị Xóa (404 Not Found):**
    ...
    ```
    *(Lưu ý: Status code của HTTP Response thực tế trả về là 404 Not Found, nhưng body status chứa 409 Conflict theo thiết kế xử lý ngoại lệ tùy chỉnh của dự án).*
...
#### Ca 1: Khách hàng tải ảnh chung (Không lưu Database)
*   **Đăng nhập với:** Tài khoản `customer_test`
*   **API:** `POST http://localhost:8080/api/v1/customer/files/upload`
*   **Headers:**
    *   `Authorization`: `Bearer <Access_Token_Của_Customer>`
*   **Body (Định dạng: form-data):**
    *   Key: `file` (chọn kiểu là **File**), Value: Chọn 1 ảnh có kích thước `< 5MB` (ví dụ định dạng PNG/JPG).
*   **Kết quả mong đợi (200 OK):** Trả về link an toàn của Cloudinary trong trường `data`.

#### Ca 2: Quản lý tải ảnh chung (Không lưu Database)
...
#### Ca 3: Admin tải ảnh và cập nhật trực tiếp cho Sân Cầu Lông (Có lưu Database)
*   **Đăng nhập với:** Tài khoản `admin`
*   **API:** `POST http://localhost:8080/api/v1/admin/files/upload/courts/{id}` (Thay `{id}` bằng `1`, `2`, `3` hoặc `4` tương ứng với dữ liệu mẫu trong DB)
*   **Headers:**
    *   `Authorization`: `Bearer <Access_Token_Của_Admin>`
*   **Body (Định dạng: form-data):**
    *   Key: `file` (chọn kiểu là **File**), Value: Chọn 1 ảnh dung lượng `< 5MB`.
*   **Kết quả mong đợi (200 OK):** Trả về link an toàn và thực hiện cập nhật cột `image_url` cho sân có ID tương ứng trong bảng `courts`.

#### Ca 4: Quản lý tải ảnh và cập nhật trực tiếp cho Sân Cầu Lông (Có lưu Database)
*   **Đăng nhập với:** Tài khoản `manager_test`
...
#### Ca 5: Tải tệp quá dung lượng tối đa (Lớn hơn 5MB)
...
#### Ca 6: Tải tệp sai định dạng hình ảnh (Không phải PNG/JPG/JPEG)
*   **API:** Gọi bất kỳ API upload nào ở trên.
*   **Body (Định dạng: form-data):**
...
#### Ca 7: Khách hàng cố tình cập nhật ảnh cho Sân Cầu Lông (Sai phân quyền)
*   **Đăng nhập với:** Tài khoản `customer_test`
*   **API:** `POST http://localhost:8080/api/v1/admin/files/upload/courts/1` hoặc `POST http://localhost:8080/api/v1/manager/files/upload/courts/1`
*   **Headers:**
    *   `Authorization`: `Bearer <Access_Token_Của_Customer>`
...
#### Ca 8: Quản lý/Admin cập nhật ảnh cho Sân không tồn tại trong DB
*   **API:** `POST http://localhost:8080/api/v1/manager/files/upload/courts/9999` (ID không tồn tại)
*   **Headers:**
    *   `Authorization`: `Bearer <Access_Token_Của_Manager/Admin>`
*   **Body (Định dạng: form-data):**

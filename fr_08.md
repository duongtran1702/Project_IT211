...
Tính năng **Phê duyệt / Từ chối lịch (FR-08)** cho phép Quản lý (`ROLE_MANAGER`) hoặc Quản trị viên (`ROLE_ADMIN`) phê duyệt hoặc từ chối các yêu cầu đặt sân của khách hàng có trạng thái đang chờ xử lý (`PENDING`).
...
1. **Phân quyền và bảo mật:** 
   - Endpoint thuộc đường dẫn `/api/v1/manager/bookings/**` yêu cầu người dùng phải đăng nhập và sở hữu vai trò `ROLE_MANAGER` hoặc `ROLE_ADMIN`.
...
*   **Method:** `PUT`
*   **Path:** `/api/v1/manager/bookings/{id}`
*   **Access:** `ROLE_MANAGER`, `ROLE_ADMIN`
*   **Body (JSON Request):**
    ```json
    {
...
### 2. Bộ điều khiển API dành cho Manager/Admin

...
*   **Access:** `ROLE_CUSTOMER` (Yêu cầu JWT Token của Khách hàng)
*   **Params:**
    *   `page`: Vị trí trang cần xem, bắt đầu từ `0` (mặc định: `0`).
    *   `size`: Số bản ghi mỗi trang (mặc định: `10`).

#### A. Trường hợp thành công (200 OK)
*   **Response (JSON):** (Sắp xếp mặc định theo ngày đặt sân giảm dần)
    ```json
    {
      "success": true,
...
#### B. Trường hợp lỗi
*   **Lỗi tham số phân trang không hợp lệ (400 Bad Request):** (Truyền tham số `page` hoặc `size` sai kiểu dữ liệu)
    ```json
    {
      "timestamp": "2026-06-11T06:05:00",
      "status": 400,
